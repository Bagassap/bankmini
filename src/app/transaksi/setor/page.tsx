...
export default function SetorTunaiPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!nasabah || !user) return;
    setSubmitting(true);
    try {
      const combinedKeterangan = [sumberDana, keterangan.trim()]
        .filter(Boolean)
        .join(" — ");
      const { data } = await api.post<Transaksi>("/transaksi/setor", {
    ...
    <Layout>
      <div className="grid grid-cols-1 gap-4 md:gap-6 2xl:gap-7.5 lg:grid-cols-[1fr_340px]">
          ...
          >
            ...
            <div className="relative mb-4 flex items-center justify-between gap-2">
              <div className="flex items-center gap-3">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-linear-to-br from-gradient-green-from to-gradient-green-to text-white shadow-sm">
                  <Banknote size={18} />
                </span>
                <div>
                  <p className="text-sm font-bold text-text-primary">Detail Setoran Tunai</p>
                  <p className="text-xs text-text-secondary">
                    {nasabah
                      ? "Pastikan nominal uang yang disetor sesuai"
            ...
            >
              <form id="setor-form" onSubmit={handleSubmit} className="flex flex-col gap-4">
                <div>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Misal: Setoran bulanan..."
                      className={inputClass}
                    />
                  </div>
                </div>
        ...
        <TransaksiRightPanel
          ...
          submitDisabled={!nasabah || submitting}
          formId="setor-form"
          onCancel={resetForm}
        />
      </div>
